...
### Step 5: Synthesize the Writing Guide

Combine analysis from all 5 papers into `WRITING_STYLE_GUIDE.md` with these sections:
...
### Step 6: Write Guardrails

The guardrails section MUST include the following rules. Write them into WRITING_STYLE_GUIDE.md under a clear heading.
...
### Step 7: Write WRITING_STYLE_GUIDE.md

Write the complete guide to `$PROJECT_DIR/WRITING_STYLE_GUIDE.md`.

The file should be self-contained: someone reading only this file should be able to write a well-structured paper in the target field.

Format:
- Use clear markdown headings with section IDs for easy reference
- Include concrete examples (good vs bad) from the reference papers
- Keep the guardrails section prominent (near the top or clearly marked)
- Total length: aim for 800-1200 lines (comprehensive but scannable)

### Step 8: Register Writing Guide in Project Agent Config

The user's project may already have `CLAUDE.md` and/or `AGENTS.md` (these are standard config files for Claude Code and Codex projects). After generating `WRITING_STYLE_GUIDE.md`, add a reference to it in both files so the agents will follow the guide when writing the paper.
...
Before writing any paper content, read `WRITING_STYLE_GUIDE.md` in full.
All guardrails in Section 5 are mandatory. Violations must be fixed before proceeding.

When using paper writing skills (/paper-plan, /paper-write, /paper-compile, /auto-paper-improvement-loop),
always check the writing guide first and follow its section-by-section recommendations.
```
...
Before drafting any paper section, read `WRITING_STYLE_GUIDE.md` completely.
All guardrails in Section 5 are mandatory — do not skip or soften them.

When writing paper content (plan, draft, compile, improve), always follow
the recommendations and constraints in WRITING_STYLE_GUIDE.md.
```

## Key Rules

- The writing guide is project-specific (different papers get different guides). It lives in the project directory, NOT in global skill directories.
...
- The guardrails (Section 5) are non-negotiable. They go into every generated guide without exception.
...
1. `WRITING_STYLE_GUIDE.md` — The complete writing style guide (800-1200 lines)
2. Updated `CLAUDE.md` — With paper writing section referencing the guide
3. Updated `AGENTS.md` — With paper writing section referencing the guide
4. Downloaded PDFs in `papers/reference_papers/`

## Composing with Other Skills

```
/paper-style-scout "direction"  →  generates WRITING_STYLE_GUIDE.md
/paper-plan                     →  reads guide when planning (if CLAUDE.md references it)
/paper-write                    →  follows guide during drafting
/paper-compile                  →  compiles to PDF
/auto-paper-improvement-loop    →  review loop respects guide guardrails
```
